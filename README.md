...
## Що перевірити перед показом

- `TEAM` може створити акаунт через сайт без ручного API.
- `ADMIN` може створити турнір, раунд і користувачів інших ролей через UI.
- `JURY` бачить призначення та зберігає оцінювання по категоріях.
- `Лідерборд` показує команди, категорії та результати по раундах.
- `npm run test`, `npm run build` і GitHub CI проходять успішно.
